...
func WriteMulti(w io.Writer, opts Options, inputs []Input) error {
	...
	for idx, in := range inputs {
		switch in.Kind {
		case TarInput:
			es, err := collectTar(in.Reader, in.Digester, idx)
			if err != nil {
				return err
			}
			collected = append(collected, es...)
		case MtreeInput:
			es, err := collectMtree(in.Reader, opts, idx)
			if err != nil {
				return err
			}
			collected = append(collected, es...)
...
type collectedEntry struct {
	...
	input   int              // index of the Input this entry came from (layer order)
}
...
// digest (obtained from digester). Every entry is tagged with the input index so
// the changeset layout can tell which layer contributed it. Hardlink grouping is
// done globally afterwards.
func collectTar(r io.Reader, digester ContentDigester, input int) ([]collectedEntry, error) {
	tr := tar.NewReader(r)
	...
	for {
		...
		out = append(out, collectedEntry{path: cleanPath(hdr.Name), hdr: hdr, digest: digest, input: input})
	}
	return out, nil
}

// mtreeEntryPath returns the full path of an mtree Entry, decoded from vis
// encoding. It replaces go-mtree's Entry.Path() method, which calls
// filepath.Clean and filepath.Join internally: on Windows those convert '/'
// to '\', corrupting forward-slash paths written by buildEntry and making the
// format platform-dependent. This version uses strings.ReplaceAll to undo any
// '\' → '/' corruption introduced by ParseSpec (parse.go:103) and path.Join
// (always '/') to reassemble relative paths.
func mtreeEntryPath(e *gomtree.Entry) (string, error) {
	name := strings.ReplaceAll(e.Name, `\`, `/`)
	decoded, err := govis.Unvis(name, gomtree.DefaultVisFlags)
	if err != nil {
		return "", err
	}
	decoded = cleanPath(decoded)
	if e.Parent == nil || e.Type == gomtree.FullType {
		return decoded, nil
	}
	parentPath, err := mtreeEntryPath(e.Parent)
	if err != nil {
		return "", err
	}
	return path.Join(parentPath, decoded), nil
}
...
// Every entry is tagged with the input index (layer order).
func collectMtree(r io.Reader, opts Options, input int) ([]collectedEntry, error) {
...
// sourceInput records the index of the input (layer) that last placed or
// synthesized the node, used to apply opaque whiteouts across layers (an opaque
// whiteout hides entries from lower layers only).
...
// missing intermediate directories (attributed to input), and returns the
// deepest one.
func (root *treeNode) ensureDir(components []string, input int) *treeNode {
	cur := root
	for _, comp := range components {
		child := cur.children[comp]
		if child == nil {
			child = &treeNode{name: comp, isDir: true, synthesized: true, sourceInput: input, children: map[string]*treeNode{}}
...
func writeChangeset(dh *gomtree.DirectoryHierarchy, collected []collectedEntry, inodes map[string]*inode, hardlinkOf map[int]*inode, opts Options) error {
	...
	for i := range collected {
		...
		case base == whiteoutOpaque:
			// Opaque whiteout: hide every entry inherited from lower layers in
			// this directory (recursively), keeping entries contributed by the
			// current layer (input) and any container directories they need. For
			// a single input (a per-layer mtree over an empty base) there is no
			// lower layer, so this is a no-op -- matching prior behavior.
			parent := root.ensureDir(parentComps, input)
			for name, child := range parent.children {
				if !removeLowerLayerEntries(child, input) {
...
// removeLowerLayerEntries prunes, from the subtree rooted at n, every node
// contributed by a layer below threshold (sourceInput < threshold), keeping any
// node from the current-or-later layer and the container directories on the path
// to it. It reports whether the subtree still contains any surviving node, so the
// caller can drop a child that has been emptied entirely. This implements the OCI
// opaque-whiteout rule: an opaque whiteout in a directory hides that directory's
// contents from lower layers only.
func removeLowerLayerEntries(n *treeNode, threshold int) bool {
	keep := n.sourceInput >= threshold
	for name, child := range n.children {
		if removeLowerLayerEntries(child, threshold) {
			keep = true // a surviving descendant keeps this container alive
		} else {
