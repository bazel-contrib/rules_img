...
func New[HM hashHelper](appender api.TarAppender, opts ...Option) *CAS[HM] {
	...
	} else if options.indexWriter != nil {
		observer = newIndexObserver[HM](options.indexWriter)
	}
...
func (c *CAS[HM]) writeHeaderAndData(hdr *tar.Header, data io.Reader, contentDigest []byte, localPathHint string) error {
	...
	if c.observer != nil {
		observerPath := localPathHint
		if !c.indexRecordLocalPaths {
			observerPath = ""
		}
		var err error
		contentSink, err = c.observer.BeginEntry(hdr, contentDigest, observerPath)
		if err != nil {
			return err
		}
	}
...
func (c *CAS[HM]) WriteRegularFromPath(hdr *tar.Header, filePath string) error {
	...
	return c.writeHeaderOrDefer(hdr, df, nil, filePath)
}
...
func (c *CAS[HM]) WriteRegularFromPathDeduplicated(hdr *tar.Header, filePath string) error {
	...
	if isBlobTarHeader(hdr) {
		linkPath, storeErr = c.storeKnownHashAndSize(df, hash, size, hdr.Name, filePath)
	} else {
		linkPath, storeErr = c.storeNodeKnownHash(df, hdr, hash, filePath)
	}
	if storeErr != nil {
		return storeErr
	}
...
func (c *CAS[HM]) storeKnownHashAndSize(r io.Reader, hash []byte, size int64, intendedPath string, localPathHint string) (string, error) {
	hashStr := string(hash)
	...
	if err := c.writeHeaderAndData(header, r, hash, localPathHint); err != nil {
		return "", err
	}
...
func (c *CAS[HM]) storeNodeKnownHash(r io.Reader, hdr *tar.Header, blobHash []byte, localPathHint string) (linkPath string, err error) {
	var helper HM
	...
	if err := c.writeHeaderAndData(&recordedTarHeader, r, blobHash, localPathHint); err != nil {
		return hdr.Name, err
	}
...
func (c *CAS[HM]) StoreFileFromPath(filePath string, intendedPath string) (string, []byte, int64, error) {
	...
	contentPath, err := c.storeKnownHashAndSize(df, hash, size, intendedPath, filePath)
...
func (c *CAS[HM]) StoreNodeFromPath(filePath string, hdr *tar.Header) (linkPath string, blobHash []byte, size int64, err error) {
	...
	linkPath, err = c.storeNodeKnownHash(df, hdr, hash, filePath)
...
func (c *CAS[HM]) writeHeaderOrDefer(hdr *tar.Header, data io.Reader, contentDigest []byte, localPathHint string) error {
	...
	return c.writeHeaderAndData(hdr, data, contentDigest, localPathHint)
}
