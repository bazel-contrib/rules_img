...
func TestIndexWriterEmptyStream(t *testing.T) {
	var buf bytes.Buffer
	iw := NewIndexWriter(&buf, IndexHashAlgoSHA256, 32, IndexStreamCompressionNone, OriginalCompressionInfo{}, 0)
	...
	if len(data) != indexHeaderSize {
		t.Fatalf("expected %d bytes (header only), got %d", indexHeaderSize, len(data))
	}

	if string(data[0:6]) != indexMagic {
		t.Fatalf("bad magic: %q", data[0:6])
	}
	if data[6] != 0x00 || data[7] != indexVersion {
		t.Fatalf("bad version bytes: %x %x", data[6], data[7])
	}
	if binary.BigEndian.Uint16(data[8:10]) != IndexHashAlgoSHA256 {
		t.Fatalf("bad hash algo")
	}
	if binary.BigEndian.Uint16(data[10:12]) != 32 {
		t.Fatalf("bad hash size")
	}
	if data[12] != IndexStreamCompressionNone {
	...
	if refTableOffset != indexHeaderSize {
		t.Fatalf("expected ref table offset %d, got %d", indexHeaderSize, refTableOffset)
	}
	if refTableSize != 0 {
		t.Fatalf("expected ref table size 0, got %d", refTableSize)
	}
	if streamOffset != indexHeaderSize {
		t.Fatalf("expected stream offset %d, got %d", indexHeaderSize, streamOffset)
	}
	if streamSize != 0 {
...
func TestIndexWriterOriginalCompressionMetadata(t *testing.T) {
	var buf bytes.Buffer
	iw := NewIndexWriter(&buf, IndexHashAlgoSHA256, 32, IndexStreamCompressionNone, OriginalCompressionInfo{
		Compression:      IndexOriginalCompressionGzip,
		Seekable:         true,
		CompressionLevel: 6,
		CompressorJobs:   4,
	}, 0)
	...
	data := buf.Bytes()
	if data[13] != IndexOriginalCompressionGzip {
		t.Fatalf("expected original compression gzip (%d), got %d", IndexOriginalCompressionGzip, data[13])
	}
	if data[14] != 1 {
...
func TestIndexWriterEndPadding(t *testing.T) {
	var buf bytes.Buffer
	iw := NewIndexWriter(&buf, IndexHashAlgoSHA256, 32, IndexStreamCompressionNone, OriginalCompressionInfo{
		EndPadding: 1024,
	}, 0)
...
func TestIndexWriterStreamBytesOnly(t *testing.T) {
	var buf bytes.Buffer
	iw := NewIndexWriter(&buf, IndexHashAlgoSHA256, 32, IndexStreamCompressionNone, OriginalCompressionInfo{}, 0)
...
func TestIndexWriterSingleCASRef(t *testing.T) {
	var buf bytes.Buffer
	iw := NewIndexWriter(&buf, IndexHashAlgoSHA256, 32, IndexStreamCompressionNone, OriginalCompressionInfo{}, 0)
...
func TestIndexWriterMultipleCASRefs(t *testing.T) {
	var buf bytes.Buffer
	iw := NewIndexWriter(&buf, IndexHashAlgoSHA256, 32, IndexStreamCompressionNone, OriginalCompressionInfo{}, 0)
...
func TestIndexWriterDigestLengthValidation(t *testing.T) {
	var buf bytes.Buffer
	iw := NewIndexWriter(&buf, IndexHashAlgoSHA256, 32, IndexStreamCompressionNone, OriginalCompressionInfo{}, 0)
...
func TestIndexWriterZstdStreamCompression(t *testing.T) {
	var buf bytes.Buffer
	iw := NewIndexWriter(&buf, IndexHashAlgoSHA256, 32, IndexStreamCompressionZstd, OriginalCompressionInfo{}, 0)
	...
	data := buf.Bytes()
	if data[12] != IndexStreamCompressionZstd {
...
func TestIndexWriterDefaultCompressionLevel(t *testing.T) {
	var buf bytes.Buffer
	iw := NewIndexWriter(&buf, IndexHashAlgoSHA256, 32, IndexStreamCompressionNone, OriginalCompressionInfo{
		Compression:      IndexOriginalCompressionGzip,
		CompressionLevel: -1,
...
func TestIndexWriterInlineThreshold(t *testing.T) {
	var buf bytes.Buffer
	iw := NewIndexWriter(&buf, IndexHashAlgoSHA256, 32, IndexStreamCompressionNone, OriginalCompressionInfo{}, 4096)
...
func TestIndexHeaderBytesCanBeParsed(t *testing.T) {
	headers := []*tar.Header{
...
func TestIndexWriterCASRefsSortedByOffset(t *testing.T) {
	var buf bytes.Buffer
	iw := NewIndexWriter(&buf, IndexHashAlgoSHA256, 32, IndexStreamCompressionNone, OriginalCompressionInfo{}, 0)
