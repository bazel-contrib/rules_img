...
def go_proto_library(*, name, compilers, filenames = [], **kwargs):
    ...
    #
    # These checks are skipped on Windows: the checked-in .pb.go files use LF line
    # endings while the bytes produced under a Windows checkout differ, causing
    # spurious diffs. Linux and macOS cover the freshness check.
