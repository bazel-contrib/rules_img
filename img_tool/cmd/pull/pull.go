...
func PullProcess(ctx context.Context, args []string) {
	...
	platformFilters, err := parsePlatforms(requestedPlatforms)
	if err != nil {
	...
	for _, registry := range registries {
		err := pullFromRegistry(ctx, registry, repository, reference, digest, store, transport, layerHandling, concurrency, platformFilters)
		lastErr = err
		if err == nil {
			return
		}
...
func pullFromRegistry(ctx context.Context, registry, repository, tag, digest string, store *blobstore.Store, transport http.RoundTripper, layerHandling string, concurrency int, platformFilters []platformFilter) error {
	...
	if desc.Descriptor.MediaType.IsImage() {
		...
		layers, err = downloadIndex(ctx, index, store, concurrency, platformFilters)
		if err != nil {
...
// platformFilter matches index children against one "os/arch[/variant]" spec. A spec that
// names no variant matches every variant of that os/architecture, so adding a filter never
// changes which child a build ends up selecting - it only drops platforms that were not
// asked for. This mirrors img/private/platforms/matching.bzl, which has to make the same
// decision for the children this tool did not prefetch.
type platformFilter struct {
	platform   ocispec.Platform
	anyVariant bool
}

func (f platformFilter) match(candidate ocispec.Platform) bool {
	candidate = platforms.Normalize(candidate)
	if candidate.OS != f.platform.OS || candidate.Architecture != f.platform.Architecture {
		return false
	}
	return f.anyVariant || candidate.Variant == f.platform.Variant
}

// parsePlatforms turns "os/arch[/variant]" specs into filters. An empty list means
// "no filter": every child of an index is downloaded.
func parsePlatforms(specs []string) ([]platformFilter, error) {
	filters := make([]platformFilter, 0, len(specs))
	for _, spec := range specs {
		...
		filters = append(filters, platformFilter{
			...
			// No compatibility vector is applied either way: asking for linux/amd64
			// must not match linux/386.
			anyVariant: strings.Count(spec, "/") < 2,
		})
	}
	return filters, nil
}

// filterManifests keeps the index children matching any of the filters. Children without
...
func filterManifests(descriptors []registryv1.Descriptor, filters []platformFilter) []registryv1.Descriptor {
	if len(filters) == 0 {
		return descriptors
	}
...
func downloadIndex(ctx context.Context, index registryv1.ImageIndex, store *blobstore.Store, concurrency int, platformFilters []platformFilter) ([]registryv1.Layer, error) {
	indexManifest, err := index.IndexManifest()
	if err != nil {
		return nil, fmt.Errorf("getting index manifest: %w", err)
	}
	descriptors := filterManifests(indexManifest.Manifests, platformFilters)
