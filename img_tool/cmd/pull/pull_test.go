...
func TestFilterManifests(t *testing.T) {
	...
	all := []registryv1.Descriptor{amd64, amd64v3, arm64v8, arm64, windows, attestation, noPlatform}
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			name:  "amd64 does not match other variants",
			specs: []string{"linux/amd64"},
			want:  []registryv1.Descriptor{amd64},
		},
		{
			name:  "multiple platforms",
			specs: []string{"linux/amd64", "windows/amd64"},
			want:  []registryv1.Descriptor{amd64, windows},
		},
		{
