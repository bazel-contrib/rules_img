...
func DownloadManifestProcess(ctx context.Context, args []string) {
	...
	flagSet.Usage = func() {
		...
		fmt.Fprintf(flagSet.Output(), "Usage: img download-manifest [OPTIONS]\n")
		flagSet.PrintDefaults()
		examples := []string{
			"img download-manifest --digest sha256:abc123... --source library/ubuntu@index.docker.io --output manifest.json",
			"img download-manifest --tag latest --source library/ubuntu@index.docker.io --source my-mirror/ubuntu@mirror.io --output manifest.json",
		}
