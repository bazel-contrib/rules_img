...
func TestTransport_InvalidDigest(t *testing.T) {
	...
	// Should fall back to base transport due to digest mismatch
	body, _ := io.ReadAll(resp.Body)
	if string(body) != "from fallback" {
		t.Errorf("Expected fallback response, got %q", body)
	}

	// Check that invalid blob was removed
	if _, err := os.Stat(blobPath); !os.IsNotExist(err) {
		t.Errorf("Invalid blob should have been removed")
	}
}
