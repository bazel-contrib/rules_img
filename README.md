...
```starlark
bazel_dep(name = "rules_img", version = "0.3.22")
```
...
# tag; "all" (default) also writes the tags, so the whole image exists in the
# registry under its tags after the build.
...
# the "blobs_and_manifests" and "all" modes. When set, the manifest(s)/index (and
# the config uploaded alongside them) are written to this repository instead of the
# image's real repository. It does not change where layer blobs are mounted from
# (that is push_at_build_time_blob_repository) and does not affect the `bazel run`
# deploy.
...
# [Experimental] When compact layers are enabled, also add a lazy action to
# every layer rule that reconstructs the real layer blob from its compact
# stream. This populates SingleLayerInfo.blob, so oci_layout, docker-save,
# image_index and image_optimize work with compact layers too. The action only
# runs if a requested target actually needs the blob -- see the "Materializing
# the blob on demand" section of docs/compact-stream.md for the added cost.
