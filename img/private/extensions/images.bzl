...
def _images_impl(ctx):
    ...
    # cas holds small blobs like config and index manifests in memory.
    # digest -> blob content
    cas = {}
    ...
    for digest, img in images_by_digest.items():
        ref_graph_entry, manifest_data = download_and_parse_manifest(ctx, digest, img, facts)
        oci_ref_graph[digest] = ref_graph_entry
        cas[digest] = manifest_data
    ...
    for digest, img in images_by_digest.items():
        ...
        image_repo(
            ...
            target_compatible_with = "[]",  # TODO: extract from platform info
        )
