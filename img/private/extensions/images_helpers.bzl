...
def download_and_parse_manifest(ctx, digest, img, facts, downloader = "img_tool"):
    ...
    Args:
        ...
        downloader: Downloader to use (default: "img_tool")
...
def download_config_blob(ctx, config_digest, img, downloader = "img_tool"):
    """Download a config blob (relies on download_blob's own caching).

    Args:
        ctx: Module extension context
        config_digest: Config digest to download
        img: Image struct with registry/repository info
        downloader: Downloader to use (default: "img_tool")
    ...
    # Download the config (download_blob has its own caching)
    registries = get_registries_from_image(img)
    config_info = download_blob(
        ctx,
        downloader = downloader,
