"""Platform matching of image manifests against a target platform.

Shared by `select_base` (which narrows an `ImageIndexInfo` base image to the manifest for the
target platform) and by the `image_platform_select` rule (which does the same for the handful
of manifests a pulled image has for one os/architecture).
"""
...
def platform_matches_exact(wanted_platform, manifest):
...
def match_manifest(manifests, os, architecture, variant):
    ...
    """
    for wanted_platform in platform_vector(os, architecture, variant):
        for manifest in manifests:
            if platform_matches_exact(wanted_platform, manifest):
                return manifest
    return None
