...
def _image_manifest_impl(ctx):
    ...
    # cmd). These are written to a JSON side file rather than passed as flags
    # because the INHERIT_FROM_BASE sentinel carries a NUL byte, which cannot
    # travel through a command line. The tool applies the inherit/unset/expand
    # semantics against the base image config (see cmd/manifest).
    config_overrides = {
        "user": ctx.attr.user,
        "workingDir": effective_working_dir,
        "stopSignal": ctx.attr.stop_signal,
        "entrypoint": effective_entrypoint,
        "cmd": effective_cmd,
    }
    config_overrides_file = ctx.actions.declare_file(ctx.label.name + "_config_overrides.json")
    ctx.actions.write(config_overrides_file, json.encode(config_overrides))
    inputs.append(config_overrides_file)
    args.add("--config-overrides", config_overrides_file.path)
...
As with Docker, setting an explicit entrypoint clears any inherited `cmd`.""",
            default = [INHERIT_FROM_BASE],
        ),
