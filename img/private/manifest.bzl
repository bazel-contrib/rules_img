...
load("//img/private/platforms:match.bzl", "match_manifest", "no_match_message")
...
def select_base(ctx):
    ...
    match = match_manifest(
        ctx.attr.base[ImageIndexInfo].manifests,
        os_wanted,
        arch_wanted,
        variant_wanted,
    )
    if match == None:
        fail(no_match_message(os_wanted, arch_wanted, variant_wanted))
    return match
