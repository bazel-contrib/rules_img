...
FIELDS = dict(
    ...
    credential_helper = "Credential helper to use for registry requests and load-strategy gRPC connections. Uses Bazel's credential-helper HTTP header protocol. WARNING: Authorization: Bearer values are treated as registry access tokens and are not exchanged as OAuth refresh tokens. Uses $IMG_CREDENTIAL_HELPER env var or tools/credential-helper if not set.",
)
