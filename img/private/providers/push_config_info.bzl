...
FIELDS = dict(
    ...
    deduplicated_push_blob_repository = "Resolved repository that every blob the deduplicated push shares between repositories is uploaded to and cross-mounted from, ahead of any home repository the deploy would have picked itself, or '' to let the deploy pick one per blob.",
    deduplicated_push_content = "Resolved deduplicated push content: 'blobs' or 'blobs_and_artificial_manifests' (which also writes a config blob and a manifest referencing the shared blob to its home repository).",
